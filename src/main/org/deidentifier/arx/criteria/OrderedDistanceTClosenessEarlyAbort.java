 ...
 */
public class OrderedDistanceTClosenessEarlyAbort extends TCloseness {
	 ...
	 * @param t
	 */
	public OrderedDistanceTClosenessEarlyAbort(String attribute, double t) {
		super(attribute, t);
	}

	@Override
	public OrderedDistanceTClosenessEarlyAbort clone() {
		return new OrderedDistanceTClosenessEarlyAbort(this.getAttribute(), this.getT());
	}
	...
	public boolean isAnonymous(Transformation node, HashGroupifyEntry entry) {
		...
		for (int i = 0; i < buckets.length; i += 2) {
			if (buckets[i] != -1) { // bucket not empty
				...
				if (Math.abs(dist) > threshold) {
					if(MEASURE_BLOCKS) TIME_BLOCK_1 += System.nanoTime() - time_blocks;
					if(MEASURE_TOTAL_TIME) TOTAL_TIME += System.nanoTime() - time_overall;
					if(COUNT_ABORTS) ABORTS++;
					return false;
				}
				map.put(value, dist);
			}
		...
		if(MEASURE_BLOCKS) TIME_BLOCK_1 += System.nanoTime() - time_blocks; time_blocks = System.nanoTime();
		...
		for (int i = 0; i < order.length; i++) {
			...
			if (distance > threshold) {
				if(MEASURE_BLOCKS) TIME_BLOCK_2 += System.nanoTime() - time_blocks;
				if(MEASURE_TOTAL_TIME) TOTAL_TIME += System.nanoTime() - time_overall;
				return false;
			}
		}
